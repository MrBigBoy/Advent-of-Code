...
["22", "52", "91", "18", "19", "14", "68", "54", "93"]
        ...
        // Step 2: Process each update and correct order if necessary
        ...
        foreach (var update in updates)
        {
            ...
            {
                // If update is incorrect, correct it
                var sortedUpdate = TopologicalSort(updatePages, orderRules);
                int middlePage = GetMiddlePage(sortedUpdate);
                Console.WriteLine($"Middle page for corrected update ({string.Join(",", update)}) is: {middlePage}");
                totalMiddlePages += middlePage;
            }
        }

        // Output the total of middle page numbers for corrected updates
        Console.WriteLine($"Total of middle pages from corrected updates: {totalMiddlePages}");
    ...
    private static bool IsUpdateCorrectlyOrdered(int[] updatePages, Dictionary<int, HashSet<int>> orderRules)
    {
        ...
        foreach (var rule in orderRules)
        {
            ...
            if (updatePages.Contains(before))
            {
                ...
                foreach (var after in afterPages.Where(after => updatePages.Contains(after)))
                {
    ...
    // Step 3: Topological sort to order pages according to the rules
    private static List<int> TopologicalSort(int[] updatePages, Dictionary<int, HashSet<int>> orderRules)
    {
        var result = new List<int>();
        var processed = new HashSet<int>();
        var dependencies = new Dictionary<int, HashSet<int>>();

        // Initialize dependencies map for all pages in the update
        foreach (var page in updatePages)
        {
            if (!dependencies.ContainsKey(page))
                dependencies[page] = []; // Initialize with an empty set if the page has no dependencies
        }

        // Add dependencies for each rule
        foreach (var rule in orderRules)
        {
            int before = rule.Key;
            foreach (var after in rule.Value)
            {
                // Only add dependencies for pages that are part of the updatePages
                if (updatePages.Contains(before) && updatePages.Contains(after))
                {
                    if (!dependencies.ContainsKey(after))
                        dependencies[after] = [];

                    _ = dependencies[after].Add(before);
                }
            }
        }

        // List of pages to process (those with no dependencies)
        var queue = new Queue<int>(updatePages.Where(p => !dependencies.ContainsKey(p) || dependencies[p].Count == 0));
        ...
        while (queue.Count != 0)
        {
            ...
            result.Add(current);
            _ = processed.Add(current);

            // Remove current from dependencies and add unprocessed pages with no dependencies
            foreach (var rule in orderRules)
            {
                if (rule.Key == current)
                {
                    foreach (var after in rule.Value.Where(after => dependencies.ContainsKey(after)))
                    {
                        _ = dependencies[after].Remove(current);
                        if (dependencies[after].Count == 0 && !processed.Contains(after))
                        {
                            queue.Enqueue(after);
                        }
                    }
        ...
        // Ensure we have processed all pages
        return result.Count != updatePages.Length
            ? throw new InvalidOperationException("Topological sort failed due to cycle or incomplete sort.")
            : result;
    }

    // Step 4: Get the middle page number from an ordered update
    private static int GetMiddlePage(List<int> updatePages)
    {
        // Ensure the updatePages array is not empty
        if (updatePages == null || updatePages.Count == 0)
        {
            throw new ArgumentException("Update pages cannot be empty.");
        }

        // Determine the middle index
        int middleIndex = updatePages.Count / 2;
